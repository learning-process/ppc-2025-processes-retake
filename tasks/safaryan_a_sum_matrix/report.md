...
В задачах вычислительной математики и обработки данных нередко требуется работа с матрицами большого размера. Одной из базовых операций является вычисление суммы элементов в каждой строке матрицы. Такая операция применяется в статистике, анализе данных, численных методах и алгоритмах машинного обучения.

При увеличении размеров матриц последовательный подход начинает работать менее эффективно, поэтому становится актуальным использование параллельных технологий для ускорения вычислений.

Цель работы: реализовать последовательную и параллельную (MPI) версии алгоритма вычисления сумм строк матрицы, а также провести анализ их производительности и эффективности.
...
Последовательная версия алгоритма выполняет прямое суммирование элементов каждой строки.
...
- Полученная сумма записывается в соответствующую позицию результирующего вектора
...
В ходе эксперимента было выполнено 22 теста, объединённых в 4 набора. Тестирование проводилось для режимов pipeline и task_run с использованием MPI и различных потоковых реализаций.
