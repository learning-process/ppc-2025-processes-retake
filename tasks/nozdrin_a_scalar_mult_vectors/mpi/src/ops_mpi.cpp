...
bool NozdrinAScalarMultVectorsMPI::RunImpl() {
    ...
    const std::uint64_t rank64 = static_cast<std::uint64_t>(rank);

    const std::uint64_t start = (rank64 * base) + std::min(rank64, rem);
    const std::uint64_t end = start + base + (rank64 < rem ? 1 : 0);
