...
Input MakeCase(const TestType &params) {
  ...
  if (label == "collinear") {
    in.points = {Point{.x = 0, .y = 0},  Point{.x = 1, .y = 0}, Point{.x = 2, .y = 0}, Point{.x = 3, .y = 0},
                 Point{.x = -1, .y = 0}, Point{.x = 2, .y = 0}, Point{.x = 0, .y = 0}};
    return in;
  }
...
const std::array<TestType, 6> kTestParam = {
    std::make_tuple(0, "empty"),     std::make_tuple(1, "single"),    std::make_tuple(0, "square_with_inside"),
    std::make_tuple(0, "collinear"), std::make_tuple(10, "random_n"), std::make_tuple(100, "random_n")};
