...
4. Для каждой строки `i` от `0` до `N - 1` вычислить начальное значение
   `row_min = generate_value(i, 0)`.
5. Для каждого столбца `j` от `1` до `N - 1` вычислить
   `val = generate_value(i, j)` и обновить минимум:
   `row_min = min(row_min, val)`.
6. Записать результат текущей строки: `out[i] = row_min`.
7. После завершения цикла вернуть `out`.
...
Итоговые измерения:
- `seq pipeline`: процессов `1`, время `0.0003336943` с, ускорение `1.00`, эффективность `N/A`.
- `seq task`: процессов `1`, время `0.0003669810` с, ускорение `1.00`, эффективность `N/A`.
- `mpi pipeline`: процессов `8`, время `0.0002130280` с, ускорение `1.57`, эффективность `19.58%`.
- `mpi task`: процессов `8`, время `0.0001784463` с, ускорение `2.06`, эффективность `25.71%`.
