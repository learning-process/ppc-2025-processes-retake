...
Сортировка данных является одной из базовых операций обработки
информации.
В работе реализован алгоритм пузырьковой сортировки в
последовательной (SEQ)
и параллельной (MPI) версиях.

Параллельная версия основана на алгоритме чёт-нечётной перестановки
(Odd-Even Transposition Sort), который позволяет распараллелить
процесс сортировки
...
4. Каждый процесс сортирует свой локальный подмассив пузырьковой
сортировкой.
5. Выполняется p+1 фаз чёт-нечётных обменов между соседними
процессами.
