...
bool LuchnikovETransmFrAllToOneGatherMPI::ValidationImpl() {
  ...
  size_t expected_size = static_cast<size_t>(input.count) * static_cast<size_t>(type_size);
...
bool LuchnikovETransmFrAllToOneGatherMPI::RunImpl() {
  ...
  if (rank == input.root) {
    ...
    std::ranges::copy(input.data, out_ptr + static_cast<ptrdiff_t>(rank * block_size));
    for (int i = 0; i < size; i++) {
      if (i != rank) {
        MPI_Recv(out_ptr + static_cast<ptrdiff_t>(i * block_size), block_size, MPI_BYTE, i, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
      }
    }
  } else {
