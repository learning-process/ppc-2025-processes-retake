...
class LuchnikovEGenerTransformFromAllToOneGatherFuncTestsSmallValues
 ...
 protected:
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    return (input_data_ > InType{0}) && (output_data > OutType{0});
  }
...
TEST_P(LuchnikovEGenerTransformFromAllToOneGatherFuncTestsSmallValues,
       SmallInputValues) {
  ExecuteTest(GetParam());
}

TEST_P(LuchnikovEGenerTransformFromAllToOneGatherFuncTestsMediumValues,
       MediumInputValues) {
  ExecuteTest(GetParam());
}

TEST_P(LuchnikovEGenerTransformFromAllToOneGatherFuncTestsLargeValues,
       LargeInputValues) {
  ExecuteTest(GetParam());
}

const std::array<TestType, 5> kTestParamSmall = {
    std::make_tuple(InType{1}, "1"), std::make_tuple(InType{2}, "2"),
    std::make_tuple(InType{3}, "3"), std::make_tuple(InType{4}, "4"),
    std::make_tuple(InType{5}, "5")};

const std::array<TestType, 5> kTestParamMedium = {
    std::make_tuple(InType{10}, "10"), std::make_tuple(InType{15}, "15"),
    std::make_tuple(InType{20}, "20"), std::make_tuple(InType{25}, "25"),
    std::make_tuple(InType{30}, "30")};

const std::array<TestType, 5> kTestParamLarge = {
    std::make_tuple(InType{50}, "50"), std::make_tuple(InType{75}, "75"),
    std::make_tuple(InType{100}, "100"), std::make_tuple(InType{125}, "125"),
    std::make_tuple(InType{150}, "150")};

const auto kTestTasksListSmall =
    std::tuple_cat(
        ppc::util::AddFuncTask<LuchnikovEGenerTransformFromAllToOneGatherMPI,
                               InType>(kTestParamSmall,
                                       PPC_SETTINGS_luchnikov_e_gener_transm_from_all_to_one_gather),
        ppc::util::AddFuncTask<LuchnikovEGenerTransformFromAllToOneGatherSEQ,
                               InType>(kTestParamSmall,
                                       PPC_SETTINGS_luchnikov_e_gener_transm_from_all_to_one_gather));
